...
const Chatbot = () => {
  ...
  // Your system prompt:
  const systemPrompt = `You are EduBot, a gamified educational chatbot inside a learning platform. Help students:
- Answer questions about Environment
- Plan study goals and weekly schedules
- Use the app's features (dashboard, rewards, tasks)
- Answer questions about the platform
Keep answers short, clear, and encourage progress. Ask follow-up questions.`;
  ...
  try {
    ...
    const conversation = [
      {
        role: "user", // system prompt goes as user role in Gemini
  ...
  return (
    <div>
      {open && (
        <div className="fixed bottom-5 right-5 w-80 bg-black/90 border-2 border-green-500 rounded-xl shadow-lg shadow-green-500/40 backdrop-blur-md p-3 flex flex-col z-50">
          ...
          <div className="h-64 overflow-y-auto p-2 border-b border-gray-700 flex-grow space-y-2">
            ...
            {loading && (
              <div className="bg-gray-800 text-white p-2 rounded-lg text-sm">Typing...</div>
            )}
          </div>
      ...
      <button
        ...
        className={`fixed bottom-5 right-5 bg-gray-900 border-2 border-green-500 text-green-500 p-3 rounded-full shadow-md shadow-green-500/30 z-40 transition-all duration-300 hover:scale-110 ${
